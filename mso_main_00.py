...
bs_machine.userParam.update({"BufferSize": int(config['StreamingFP']['BufferSize'])})
...
class Window(QtGui.QTabWidget):
    ...
    def __init__(self, parent = None):
        ...
        self.setGeometry( \
            int(config['StreamingFP']['GeomPosX']), \
            int(config['StreamingFP']['GeomPosY']) , \
            int(config['StreamingFP']['GeomWidth']), \
            int(config['StreamingFP']['GeomLength']))
        ...
        self.plotList=[]
        self.plotNameList=[]
        self.setzone = int(config['StreamingFP']['SetZone'])
        self.sample = int(config['StreamingFP']['PlotSample'])
        self.target = 0
    ...
    def tab2UI(self):
        ...
        self.win2.addItem(self.label2)
        self.btnTestOnTab2 = QtGui.QPushButton("demoButton",self)
        layout.addWidget(self.win2,1,0,12,1)
        layout.addWidget(self.btnTestOnTab2,1,1,1,1)
    ...
    def tab1UI(self):
        ...
        self.comboBoxCOM.addItems(bs_machine.findBS())
        ...
        self.btnRunning.setFixedWidth(int(config['StreamingFP']['LogicBtnWidth']))
        ...
        self.btnL0.setFixedWidth(int(config['StreamingFP']['LogicBtnWidth']))
        self.btnL1.setFixedWidth(int(config['StreamingFP']['LogicBtnWidth']))
        self.btnL2.setFixedWidth(int(config['StreamingFP']['LogicBtnWidth']))
        self.btnL3.setFixedWidth(int(config['StreamingFP']['LogicBtnWidth']))
        self.btnL4.setFixedWidth(int(config['StreamingFP']['LogicBtnWidth']))
        self.btnL5.setFixedWidth(int(config['StreamingFP']['LogicBtnWidth']))
        self.btnL6.setFixedWidth(int(config['StreamingFP']['LogicBtnWidth']))
        self.btnL7.setFixedWidth(int(config['StreamingFP']['LogicBtnWidth']))
        ...
        self.dialSR.setMinimum(int(config['StreamingFP']['SampleRateMin']))
        self.dialSR.setMaximum(int(config['StreamingFP']['SampleRateMax']))
        self.spinSR = QtGui.QSpinBox()
        self.spinSR.setMinimum(int(config['StreamingFP']['SampleRateMin']))
        self.spinSR.setMaximum(int(config['StreamingFP']['SampleRateMax']))
        ...
        self.dialDuration.setMinimum(int(config['StreamingFP']['DurationMin']))
        self.dialDuration.setMaximum(int(config['StreamingFP']['DurationMax']))
        self.spinDuration.setMinimum(int(config['StreamingFP']['DurationMin']))
        self.spinDuration.setMaximum(int(config['StreamingFP']['DurationMax']))
...
########### CHA and CHB #####################################
    def btnChAPressed(self):
        ...
        if mode.startswith("Single"):
            self.btnChB.toggle()
            bs_machine.userParam.update({"isDual":False})
            bs_machine.userParam.update({"CHA":True})
            bs_machine.userParam.update({"CHB":False})
        else:
            print('else')
    ...
    def btnChBPressed(self):
        ...
        if mode.startswith("Single"):
            self.btnChA.toggle()
            bs_machine.userParam.update({"isDual":False})
            bs_machine.userParam.update({"CHA":False})
...
###########  MODE  ##############################
    def modebtnPressed(self):
        ...
        bs_machine.userParam.update({"testMode":mode})
        ...
        bs_machine.userParam.update({"L0": False})
        bs_machine.userParam.update({"L1": False})
        bs_machine.userParam.update({"L2": False})
        bs_machine.userParam.update({"L3": False})
        bs_machine.userParam.update({"L4": False})
        bs_machine.userParam.update({"L5": False})
        bs_machine.userParam.update({"L6": False})
        bs_machine.userParam.update({"L7": False})
        ...
        if mode == "Dual" or mode=="Mixed" or mode=="DualMacro":
            self.status.clear()
            self.plotNameList= []
            self.btnChA.setCheckable(True)
            self.btnChB.setCheckable(True)
            ...
            bs_machine.userParam.update({"isDual":True})
            bs_machine.userParam.update({"CHA":True})
            bs_machine.userParam.update({"CHB":True})
            ...
            self.dialSR.setMinimum(int(config['StreamingFP']['DualMin']))
            self.spinSR.setMinimum(int(config['StreamingFP']['DualMin']))

        elif mode == "SingleFast" or mode == "SingleMacro":
            self.status.clear()
            self.plotNameList= []
            self.status.addItem("Running in " + mode + " mode")
            ...
            if mode == "SingleFast":
                self.dialSR.setMinimum(int(config['StreamingFP']['SingleFastMin']))
                self.spinSR.setMinimum(int(config['StreamingFP']['SingleFastMin']))
            elif mode =="SingleMacro":
                self.dialSR.setMinimum(int(config['StreamingFP']['SingleMacroMin']))
                self.spinSR.setMinimum(int(config['StreamingFP']['SingleMacroMin']))
...
###########  Other Setting Buttons ###################################
    ...
    def findBS(self):
        self.comboBoxCOM.addItem(bs_machine.userParam["BSModel1"])
...
###########  START Button ##########################################
    ...
    def startBtnPressed(self, down):
        ...
        print('ssf',self.isInitialRunExecuted)
        ...
        if self.btnStart.text() == "START":   #bug:once running this won't update?
            print('text is start')
            self.addPlot()
            self.captureSigUpdatePlot()

            self.status.addItem("Sample Rate: " + \
                "{0:.2f}".format((bs_machine.summaryDict["sampleRate"])) + "Ksps")
            ...
            if bs_machine.userParam["duration"]>0:
                self.btnStart.setText("START") #stopped already
                self.btnFrameUpdate.setEnabled(False)
                self.btnToggle.setEnabled(True)
            else:
                # self.btnStart.setText("STOP") #running
                self.btnFrameUpdate.setEnabled(True)
                self.btnStart.setEnabled(False)
                self.btnToggle.setEnabled(False)
...
###########  Parameter update when Start ##########################################
    def updateUserParam(self):
        self.plotNameList=[]
        self.plotNameList.append("CHA") if bs_machine.userParam["CHA"] else None
        self.plotNameList.append("CHB") if bs_machine.userParam["CHB"] else None
        self.plotNameList.append("L0") if bs_machine.userParam["L0"] else None
        self.plotNameList.append("L1") if bs_machine.userParam["L1"] else None
        self.plotNameList.append("L2") if bs_machine.userParam["L2"] else None
        self.plotNameList.append("L3") if bs_machine.userParam["L3"] else None
        self.plotNameList.append("L4") if bs_machine.userParam["L4"] else None
        self.plotNameList.append("L5") if bs_machine.userParam["L5"] else None
        self.plotNameList.append("L6") if bs_machine.userParam["L6"] else None
        self.plotNameList.append("L7") if bs_machine.userParam["L7"] else None
        self.status.addItem(str(self.plotNameList))
...
###########  TOGGLE Button ###################################
    ...
    def timerToggle(self):
        if self.btnFrameUpdate.text() == "CONT_ON":
            self.status.addItem('Cont_caputre with a frame rate: '+ (config['StreamingFP']['FrameRate'])+ "/s")
            self.btnFrameUpdate.setText('CONT_OFF')
            timer.timeout.connect(self.updatePlot)
            timer.start(int(config['StreamingFP']['FrameRate']))
        else:
...
###########  PLOT HELPERS ###################################
    ...
    def update(self,target):
        self.region.setZValue(int(config['StreamingFP']['SetZValue']))
    ...
    def plotRealDual(self,data):
        ...
        if bs_machine.userParam["testMode"]=="Mixed":
            self.plotList[0].plot(data["chA"], pen='y')
            self.plotList[1].plot(data["chB"], pen='g')
            self.plotList[2].plot(data["logic"], pen='c')
            self.plotList[len(self.plotNameList)-1].plot(data["chA"], pen='y')
            for cnt in data["chA"]:
            ...
            self.plotList[0].plot(data["chA"][int(config['StreamingFP']['RubbishSize']):], pen='ye')
            self.plotList[len(self.plotNameList)-1].plot(data["chA"][int(config['StreamingFP']['RubbishSize']):], pen='ye')
            for cnt in data["chA"]:
                self.plotData1.append(cnt)
        else:
            self.plotList[0].plot(data["chA"][int(config['StreamingFP']['RubbishSize']):], pen='g')
            self.plotList[len(self.plotNameList)-1].plot(data["chA"][int(config['StreamingFP']['RubbishSize']):], pen='g')
            for cnt in data["chA"]:
                self.plotData1.append(cnt)
...
###########  CAPTURE SIG AND PLOT ###############################
    def captureSigUpdatePlot(self):

        self.btnRst.setEnabled(True)
        self.addPlot()
        bs_machine.setupBS() if self.isRunning == False else None
        ...
        if self.spinDuration.value()>0:
            ...
            self.plotRealDual(bs_machine.getStreamFast(int(config['StreamingFP']['FastSampleSize'])))
    ...
    def updatePlot(self):
        ...
        if self.isRunning:
            ...
            if bs_machine.userParam["isDual"]:
                ...
                self.plotRealDual(bs_machine.getStreamFast(int(config['StreamingFP']['ToGetAtATime'])))
                self.btnRunning.setIcon(QtGui.QIcon('./Images/ledON.jpeg'))
                self.isRunning = True
